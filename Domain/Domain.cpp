...
int Domain::initialize_load() {
    auto code = 0;
    for_each(load_pond.cbegin(), load_pond.cend(), [&](const std::pair<unsigned, shared_ptr<Load>>& t_load) {
        if(!t_load.second->initialized && current_step_tag >= t_load.second->get_start_step()) code += t_load.second->initialize(shared_from_this());
    });
    return code;
}
...
void Domain::assemble_mass() const {
    ...
    if(factory->get_storage_scheme() == StorageScheme::SPARSE || factory->get_storage_scheme() == StorageScheme::SPARSESYMM) std::dynamic_pointer_cast<SparseMat<double>>(factory->get_mass())->triplet_to_csc();
}
...
void Domain::assemble_initial_stiffness() const {
    ...
    if(factory->get_storage_scheme() == StorageScheme::SPARSE || factory->get_storage_scheme() == StorageScheme::SPARSESYMM) std::dynamic_pointer_cast<SparseMat<double>>(factory->get_stiffness())->triplet_to_csc();
}
...
void Domain::assemble_damping() const {
    ...
    if(factory->get_storage_scheme() == StorageScheme::SPARSE || factory->get_storage_scheme() == StorageScheme::SPARSESYMM) std::dynamic_pointer_cast<SparseMat<double>>(factory->get_damping())->triplet_to_csc();
}
