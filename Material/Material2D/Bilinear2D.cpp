...
int Bilinear2D::update_trial_status(const vec& t_strain) {
    ...
    if(plane_type == PlaneType::S) {
        auto& stress_anchor = base.get_trial_stress().at(2);
        auto counter = 0;
        while(true) {
            if(fabs(stress_anchor) < 1E-12) break;
            if(++counter > 20) {
                suanpan_warning("cannot converge in 20 iterations.\n");
                break;
            }
            trial_full_strain(2) -= stress_anchor / base.get_trial_stiffness().at(2, 2);
            base.update_trial_status(trial_full_strain);
        }
    }

    for(auto I = 0; I < 3; ++I) trial_stress(I) = base.get_trial_stress().at(F[I]);

    for(auto I = 0; I < 3; ++I)
        for(auto J = 0; J < 3; ++J) trial_stiffness(I, J) = base.get_trial_stiffness().at(F[I], F[J]);
