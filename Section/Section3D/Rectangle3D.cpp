...
Rectangle3D::IntegrationPoint::IntegrationPoint(const double C, const double W, unique_ptr<Material>&& M)
    : coor(C)
    , weight(W)
    , s_material(move(M)) {}

Rectangle3D::IntegrationPoint::IntegrationPoint(const IntegrationPoint& old_obj)
    : coor(old_obj.coor)
    , weight(old_obj.weight)
    , s_material(old_obj.s_material->get_copy()) {}

Rectangle3D::Rectangle3D(const unsigned T, const double B, const double H, const unsigned M, const unsigned S)
    : Section3D(T, ST_RECTANGLE3D, M, B * H)
    , width(B)
    , height(H)
    , int_pt_num(S) {}
...
double Rectangle3D::get_parameter(const ParameterType& P) {
    switch(P) {
...
int Rectangle3D::update_trial_status(const vec& t_deformation) {
    ...
    for(const auto& I : int_pt) {
        if(I.s_material->update_trial_status(trial_deformation(0) - trial_deformation(1) * I.coor) != SUANPAN_SUCCESS) return SUANPAN_FAIL;
