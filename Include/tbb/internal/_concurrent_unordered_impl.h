    ...
    namespace internal {
        ...
        public:
            ...
            template <typename __TBB_PARAMETER_PACK Args> nodeptr_t create_node_v(__TBB_FORWARDING_REF(Args) __TBB_PARAMETER_PACK args) {
                ...
                // TODO: use RAII scoped guard instead of explicit catch
                __TBB_TRY { new(static_cast<void*>(&pnode->my_element)) T(__TBB_PACK_EXPANSION(tbb::internal::forward<Args>(args))); }
                __TBB_CATCH(...) {
                    my_node_allocator.deallocate(pnode, 1);
                    __TBB_RETHROW();
                }
        ...
        private:
            ...
            struct call_internal_clear_on_exit {
                ...
                ~call_internal_clear_on_exit() {
                    if(my_instance) { my_instance->internal_clear(); }
                }
            };
        ...
        private:
            ...
            template <typename AllowCreate, typename ValueType> std::pair<iterator, bool> internal_insert(__TBB_FORWARDING_REF(ValueType) value, nodeptr_t pnode = NULL) {
                ...
                for(raw_iterator where = previous;;) {
                    ...
                    if(where == last || solist_t::get_order_key(where) > order_key ||
                        ...
                        (allow_multimapping && solist_t::get_order_key(where) == order_key && !my_hash_compare(get_key(*where), *pkey))) // TODO: fix negation
                    {
                        if(!pnode) {
                    ...
                    } else if(!allow_multimapping && solist_t::get_order_key(where) == order_key && !my_hash_compare(get_key(*where), *pkey)) // TODO: fix negation
            ...
            pairii_t internal_equal_range(const key_type& key) {
                ...
                for(raw_iterator it = prepare_bucket(hash_key); it != end_it; ++it) {
                    ...
                    } else if(solist_t::get_order_key(it) == order_key && !my_hash_compare(get_key(*it), key)) // TODO: fix negation; also below
                    {
