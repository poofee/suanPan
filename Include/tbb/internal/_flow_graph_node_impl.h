...
template <typename Input, typename A, typename ImplType> class function_input_base : public receiver<Input>, tbb::internal::no_assign {
    enum op_type {
        reg_pred,
        rem_pred,
        app_body,
...
private:
    ...
    void handle_operations(operation_type* op_list) {
        ...
        while(op_list) {
            ...
            case app_body:
                ...
                --my_concurrency;
                __TBB_store_with_release(tmp->status, SUCCEEDED);
                if(my_concurrency < my_max_concurrency) { create_and_spawn_task(/*spawn=*/true); }
                break;
            case app_body_bypass: {
                tmp->bypass_t = NULL;
                __TBB_ASSERT(my_max_concurrency != 0, NULL);
                --my_concurrency;
                if(my_concurrency < my_max_concurrency) tmp->bypass_t = create_and_spawn_task(/*spawn=*/false);
    ...
    void internal_forward(operation_type* op) {
        op->bypass_t = NULL;
        if(my_concurrency < my_max_concurrency || !my_max_concurrency) op->bypass_t = create_and_spawn_task(/*spawn=*/false);
        if(op->bypass_t)
            __TBB_store_with_release(op->status, SUCCEEDED);
        else {
            forwarder_busy = false;
    ...
    inline task* create_body_task(const input_type& input) { return (internal::is_graph_active(my_graph_ref)) ? new(task::allocate_additional_child_of(*(my_graph_ref.root_task()))) apply_body_task_bypass<class_type, input_type>(*this, input) : NULL; }
    ...
    inline task* create_forward_task() { return (internal::is_graph_active(my_graph_ref)) ? new(task::allocate_additional_child_of(*(my_graph_ref.root_task()))) forward_task_bypass<class_type>(*this) : NULL; }
...
template <typename Input, typename Output, typename A> class function_input : public function_input_base<Input, A, function_input<Input, Output, A>> {
public:
    typedef Input input_type;
...
template <> struct extract_element<1> {
    template <typename P> static void extract_this(P& p) { (void)tbb::flow::get<0>(p).successors().built_successors().sender_extract(tbb::flow::get<0>(p)); }
};
#endif

//! Implements methods for a function node that takes a type Input as input
//  and has a tuple of output ports specified.
template <typename Input, typename OutputPortSet, typename A> class multifunction_input : public function_input_base<Input, A, multifunction_input<Input, OutputPortSet, A>> {
public:
...
inline void check_task_and_spawn(graph& g, task* t) {
    if(t && t != SUCCESSFULLY_ENQUEUED) { internal::spawn_in_graph_arena(g, *t); }
}
...
protected:
    ...
    task* execute() __TBB_override { return (internal::is_graph_active(my_graph_ref)) ? new(task::allocate_additional_child_of(*(my_graph_ref.root_task()))) apply_body_task_bypass<continue_input<Output>, continue_msg>(*this, continue_msg()) : NULL; }
...
public:
    ...
    void internal_add_built_successor(successor_type& r) __TBB_override { successors().internal_add_built_successor(r); }

    void internal_delete_built_successor(successor_type& r) __TBB_override { successors().internal_delete_built_successor(r); }

    size_t successor_count() __TBB_override { return successors().successor_count(); }

    void copy_successors(successor_list_type& v) __TBB_override { successors().copy_successors(v); }
#endif /* TBB_PREVIEW_FLOW_GRAPH_FEATURES */
