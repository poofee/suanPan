    ...
    namespace internal {
        ...
        template <typename Partition> struct proportional_mode : adaptive_mode<Partition> {
            ...
            size_t do_split(proportional_mode& src, const proportional_split& split_obj) {
                ...
                size_t portion = size_t(float(src.my_divisor) * float(split_obj.right()) / float(split_obj.left() + split_obj.right()) + 0.5f);
#else
...
#pragma warning(pop)
        ...
        template <typename Partition> struct linear_affinity_mode : proportional_mode<Partition> {
            ...
            void set_affinity(task& t) {
                if(self().my_divisor) t.set_affinity(affinity_id(my_head) + 1);
            }
        };
        ...
        public:
            ...
            void note_affinity(task::affinity_id id) {
                if(my_divisor) my_array[my_head] = id;
            }
        };
