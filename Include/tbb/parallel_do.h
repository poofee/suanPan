...
namespace interface9 {
    ...
    namespace internal {
        ...
        template <class Body, typename Item> class parallel_do_feeder_impl : public parallel_do_feeder<Item> {
            ...
            void internal_add_copy_impl(std::false_type, const Item&) { __TBB_ASSERT(false, "Overloading for r-value reference doesn't work or it's not movable and not copyable object"); }
        ...
        template <typename Body, typename Item> class do_group_task_input : public task {
            ...
            ~do_group_task_input() {
                for(size_t k = 0; k < my_size; ++k) (my_arg.begin() + k)->~Item();
            }
