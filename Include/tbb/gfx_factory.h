    ...
    namespace interface9 {
        ...
        namespace gfx_offload {
            ...
            inline void check_enqueue_retcode(task_id_type err) {
                if(err == 0) { throw_gfx_exception(); }
            }

            inline void check_gfx_retcode(task_id_type err) {
                if(err != GFX_SUCCESS) { throw_gfx_exception(); }
            }
            ...
            template <typename DataType, typename SizeType> void share(DataType* p, SizeType n) { check_gfx_retcode(_GFX_share(p, sizeof(*p) * n)); }
            template <typename DataType> void unshare(DataType* p) { check_gfx_retcode(_GFX_unshare(p)); }

            // Retrieving array pointer from shared gfx_buffer
            // Other types remain the same
            template <typename T> T* raw_data(gfx_buffer<T>& buffer) { return buffer.data(); }
            template <typename T> const T* raw_data(const gfx_buffer<T>& buffer) { return buffer.data(); }
            template <typename T> T& raw_data(T& data) { return data; }
            template <typename T> const T& raw_data(const T& data) { return data; }

            // Kernel enqueuing on device with arguments
            template <typename F, typename... ArgType> task_id_type run_kernel(F ptr, ArgType&... args) {
        ...
        public:
            ...
            gfx_buffer()
                : my_vector_ptr(std::make_shared<std::vector<T>>()) {}
            gfx_buffer(size_type size)
                : my_vector_ptr(std::make_shared<std::vector<T>>(size)) {}
        ...
        template <typename T> class gfx_async_msg : public tbb::flow::async_msg<T> {
        public:
        ...
        private:
            ...
            class func_wrapper {
            public:
                template <typename F> func_wrapper(F ptr) { my_ptr = reinterpret_cast<void*>(ptr); }
        ...
        public:
            ...
            template <typename... Args> void send_data(device_type /*device*/, Args&... args) { send_data_impl(args...); }
            ...
            template <typename FinalizeFn, typename... Args> void finalize(device_type /*device*/, FinalizeFn fn, Args&... /*args*/) { fn(); }

            // Empty device selector.
            // No way to choose a device with GFX API.
            class dummy_device_selector {
            public:
                device_type operator()(gfx_factory& /*factory*/) { return device_type(); }
            };
        ...
        private:
            ...
            template <typename... Args> void callback(kernel_id_type id, Args... args) {
            ...
            template <typename T> void share_data(T) {}

            template <typename T> void share_data(gfx_buffer<T>& buffer) { gfx_offload::share(buffer.data(), buffer.size()); }

            template <typename T> void send_arg(T) {}

            template <typename T> void send_arg(async_msg_type<T>& msg) { share_data(msg.data()); }

            void send_data_impl() {}

            template <typename T, typename... Rest> void send_data_impl(T& arg, Rest&... args) {
                send_arg(arg);
                send_data_impl(args...);
            }
            ...
            template <typename T> void set_kernel_id_arg(kernel_id_type, T) {}

            template <typename T> void set_kernel_id_arg(kernel_id_type id, async_msg_type<T>& msg) { msg.set_task_id(id); }

            void set_kernel_id(kernel_id_type) {}

            template <typename T, typename... Rest> void set_kernel_id(kernel_id_type id, T& arg, Rest&... args) {
            ...
            template <typename T> void unshare_data(T) {}

            template <typename T> void unshare_data(gfx_buffer<T>& buffer) { gfx_offload::unshare(buffer.data()); }

            template <typename T> void receive_arg(T) {}

            template <typename T> void receive_arg(async_msg_type<T>& msg) {
            ...
            template <typename T, typename... Rest> void receive_data(T& arg, Rest&... args) {
                receive_arg(arg);
                receive_data(args...);
            }
