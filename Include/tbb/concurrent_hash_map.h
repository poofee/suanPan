...
namespace interface5 {
    ...
    namespace internal {
        ...
        public:
            ...
            hash_map_base() {
                std::memset(this, 0,
                    pointers_per_table * sizeof(segment_ptr_t) // 32*4=128   or 64*8=512
            ...
            static void init_buckets(segment_ptr_t ptr, size_type sz, bool is_initial) {
                if(is_initial)
                    std::memset(ptr, 0, sz * sizeof(bucket));
                else
            ...
            segment_index_t insert_new_node(bucket* b, node_base* n, hashcode_t mask) {
                ...
                if(sz >= mask) {                                    // TODO: add custom load_factor
                    ...
                    if(!itt_hide_load_word(my_table[new_seg]) && as_atomic(my_table[new_seg]).compare_and_swap(is_allocating, NULL) == NULL) return new_seg; // The value must be processed
                }
                return 0;
            }
        ...
        /** Value is either the T or const T type of the container.
            @ingroup containers */
        template <typename Container, typename Value> class hash_map_iterator : public std::iterator<std::forward_iterator_tag, Value> {
            ...
            template <typename Key, typename T, typename HashCompare, typename A> friend class interface5::concurrent_hash_map;
#else
        public: // workaround
#endif
        ...
        hash_map_iterator<Container, Value>::hash_map_iterator(const Container& map, size_t index, const bucket* b, node_base* n)
            ...
            if(b && !hash_map_base::is_valid(n)) advance_to_next_bucket();
        }
        ...
        template <typename Iterator> void hash_map_range<Iterator>::set_midpoint() const {
            ...
            __TBB_ASSERT(my_begin.my_index <= my_midpoint.my_index, "my_begin is after my_midpoint");
            __TBB_ASSERT(my_midpoint.my_index <= my_end.my_index, "my_midpoint is after my_end");
            __TBB_ASSERT(my_begin != my_midpoint || my_begin == my_end, "[my_begin, my_midpoint) range should not be empty");
        }
...
#pragma warning(disable : 4127)
    ...
    protected:
        ...
        struct node : public node_base {
            ...
            node(value_type& i)
                : item(const_cast<const value_type&>(i)) {}
    ...
    template <typename Key, typename T, typename HashCompare, typename A> void concurrent_hash_map<Key, T, HashCompare, A>::rehash(size_type sz) {
        ...
        if(!reported && buckets >= 512 && (2 * empty_buckets > current_size || 2 * overpopulated_buckets > current_size)) {
            tbb::internal::runtime_warning("Performance is not optimal because the hash function produces bad randomness in lower bits in %s.\nSize: %d  Empties: %d  Overlaps: %d",
