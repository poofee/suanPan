...
#error Do not know syntax for forcing alignment.
    ...
    template <> struct atomic_rep<1> { // Specialization
        typedef int8_t word;
    };
    template <> struct atomic_rep<2> { // Specialization
        typedef int16_t word;
    };
    template <> struct atomic_rep<4> { // Specialization
#if _MSC_VER && !_WIN64
    ...
    template <> struct atomic_rep<8> { // Specialization
...
#define ATOMIC_STORAGE_PARTIAL_SPECIALIZATION(S)                           \
    template <typename value_type> struct aligned_storage<value_type, S> { \
...
#else
#define ATOMIC_STORAGE_PARTIAL_SPECIALIZATION(S) \
    template <typename value_type> struct aligned_storage<value_type, S> { __TBB_DECL_ATOMIC_FIELD(value_type, my_value, S) };
...
#define __TBB_DECL_FENCED_ATOMIC_PRIMITIVES(S, M)                                                                                                                           \
    template <> struct atomic_traits<S, M> {                                                                                                                                \
        ...
        inline static word compare_and_swap(volatile void* location, word new_value, word comparand) { return __TBB_machine_cmpswp##S##M(location, new_value, comparand); } \
        inline static word fetch_and_add(volatile void* location, word addend) { return __TBB_machine_fetchadd##S##M(location, addend); }                                   \
        inline static word fetch_and_store(volatile void* location, word value) { return __TBB_machine_fetchstore##S##M(location, value); }                                 \
    };

#define __TBB_DECL_ATOMIC_PRIMITIVES(S)                                                                                                                                  \
    template <memory_semantics M> struct atomic_traits<S, M> {                                                                                                           \
        typedef atomic_rep<S>::word word;                                                                                                                                \
        inline static word compare_and_swap(volatile void* location, word new_value, word comparand) { return __TBB_machine_cmpswp##S(location, new_value, comparand); } \
        inline static word fetch_and_add(volatile void* location, word addend) { return __TBB_machine_fetchadd##S(location, addend); }                                   \
        inline static word fetch_and_store(volatile void* location, word value) { return __TBB_machine_fetchstore##S(location, value); }                                 \
    };
...
#define __TBB_DECL_ATOMIC_LOAD_STORE_PRIMITIVES(M)                                                                          \
    template <> struct atomic_load_store_traits<M> {                                                                        \
        template <typename T> inline static T load(const volatile T& location) { return __TBB_load_##M(location); }         \
        template <typename T> inline static void store(volatile T& location, T value) { __TBB_store_##M(location, value); } \
    }
...
// on suncc for 64 bit types for 32 bit target
    ...
    public:
        ...
        constexpr atomic_impl(value_type value)
            : my_storage(value) {}
#endif
        template <memory_semantics M> value_type fetch_and_store(value_type value) { return to_value<value_type>(internal::atomic_traits<sizeof(value_type), M>::fetch_and_store(&my_storage.my_value, to_bits(value))); }

        value_type fetch_and_store(value_type value) { return fetch_and_store<full_fence>(value); }

        template <memory_semantics M> value_type compare_and_swap(value_type value, value_type comparand) { return to_value<value_type>(internal::atomic_traits<sizeof(value_type), M>::compare_and_swap(&my_storage.my_value, to_bits(value), to_bits(comparand))); }
        ...
        operator value_type() const volatile { // volatile qualifier here for backwards compatibility
            return to_value<value_type>(__TBB_load_with_acquire(to_bits_ref(my_storage.my_value)));
        }

        template <memory_semantics M> value_type load() const { return to_value<value_type>(internal::atomic_load_store_traits<M>::load(to_bits_ref(my_storage.my_value))); }
