...
namespace interface5 {
    namespace internal {
#if __TBB_CPP11_IS_COPY_CONSTRUCTIBLE_PRESENT
        template <typename T, bool C = std::is_copy_constructible<T>::value> struct use_element_copy_constructor { typedef tbb::internal::true_type type; };
        template <typename T> struct use_element_copy_constructor<T, false> { typedef tbb::internal::false_type type; };
#else
        template <typename> struct use_element_copy_constructor { typedef tbb::internal::true_type type; };
#endif
    } // namespace internal
    ...
    public:
        ...
        explicit concurrent_priority_queue(const concurrent_priority_queue& src)
            : mark(src.mark)
            , my_size(src.my_size)
    ...
    private:
        ...
        void handle_operations(cpq_operation* op_list) {
            ...
            while(op_list) {
                ...
                if(tmp->type == POP_OP) {
                    if(mark < data.size() && compare(data[0], data[data.size() - 1])) {
        ...
        void push_back_helper(const T&, tbb::internal::false_type) { __TBB_ASSERT(false, "The type is not copy constructible. Copying push operation is impossible."); }
    };
