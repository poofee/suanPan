...
inline int32_t __TBB_machine_cmpswp4(volatile void* ptr, int32_t value, int32_t comparand) {
    ...
    __asm__ __volatile__("sync\n"
                         ...
                         "1:\n\t"                     /* the exit */
                         "isync"
                         : [res] "=&r"(result), "+m"(*(int32_t*)ptr) /* redundant with "memory" */
...
inline int64_t __TBB_machine_cmpswp8(volatile void* ptr, int64_t value, int64_t comparand) {
    ...
    __asm__ __volatile__("sync\n"
                         ...
                         "1:\n\t"                     /* the exit */
                         "isync"
                         : [res] "=&r"(result), "+m"(*(int64_t*)ptr) /* redundant with "memory" */
                         : [ptr] "r"(ptr), [val] "r"(value), [cmp] "r"(comparand)
                         : "memory" /* compiler full fence */
                         ,
...
inline int64_t __TBB_machine_cmpswp8(volatile void* ptr, int64_t value, int64_t comparand) {
    ...
    __asm__ __volatile__("sync\n\t"
                         ...
                         "std %[res],%[resm]\n\t"
                         "isync"
                         : [resm] "=m"(result), [res] "=&r"(result_register), [val] "=&r"(value_register), [cmp] "=&r"(comparand_register), "+m"(*(int64_t*)ptr) /* redundant with "memory" */
...
#define __TBB_MACHINE_DEFINE_LOAD_STORE(S, ldx, stx, cmpx)                                      \
    template <typename T> struct machine_load_store<T, S> {                                     \
        ...
        static inline void store_with_release(volatile T& location, T value) {                  \
            __asm__ __volatile__("lwsync\n\t" stx " %[val],0(%[ptr])"                           \
                                 ...
                                 : "memory" /*compiler release fence*/ /*(cr0 not affected)*/); \
                                                                                                ...
                                                                                                \
    template <typename T> struct machine_load_store_relaxed<T, S> {                             \
