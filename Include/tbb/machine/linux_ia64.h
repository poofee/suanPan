...
#define __TBB_MACHINE_DEFINE_ATOMICS(S, V)                                                                        \
    template <typename T> struct machine_load_store_relaxed<T, S> {                                               \
        static inline T load(const T& location) { return (T)__TBB_machine_load##S##_relaxed(&location); }         \
        static inline void store(T& location, T value) { __TBB_machine_store##S##_relaxed(&location, (V)value); } \
    }
