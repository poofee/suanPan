...
namespace flow {
    ...
    namespace interface10 {
        ...
        template <typename Factory> class opencl_program;
        ...
        template <typename T> T event_info(cl_event e, cl_event_info i) {
            T res;
            enforce_cl_retcode(clGetEventInfo(e, i, sizeof(res), &res, NULL), "Failed to get OpenCL event information");
            return res;
        }

        template <typename T> T device_info(cl_device_id d, cl_device_info i) {
            T res;
            enforce_cl_retcode(clGetDeviceInfo(d, i, sizeof(res), &res, NULL), "Failed to get OpenCL device information");
            return res;
        }

        template <> inline std::string device_info<std::string>(cl_device_id d, cl_device_info i) {
            size_t required;
        ...
        template <typename T> T platform_info(cl_platform_id p, cl_platform_info i) {
            T res;
            enforce_cl_retcode(clGetPlatformInfo(p, i, sizeof(res), &res, NULL), "Failed to get OpenCL platform information");
            return res;
        }

        template <> inline std::string platform_info<std::string>(cl_platform_id p, cl_platform_info i) {
            size_t required;
        ...
        public:
            ...
            std::string platform_profile() const { return platform_info<std::string>(platform(), CL_PLATFORM_PROFILE); }
            std::string platform_version() const { return platform_info<std::string>(platform(), CL_PLATFORM_VERSION); }
            std::string platform_name() const { return platform_info<std::string>(platform(), CL_PLATFORM_NAME); }
            std::string platform_vendor() const { return platform_info<std::string>(platform(), CL_PLATFORM_VENDOR); }
            std::string platform_extensions() const { return platform_info<std::string>(platform(), CL_PLATFORM_EXTENSIONS); }

            template <typename T> void info(cl_device_info i, T& t) const { t = device_info<T>(my_cl_device_id, i); }
            ...
            std::array<size_t, 3> max_work_item_sizes() const { return device_info<std::array<size_t, 3>>(my_cl_device_id, CL_DEVICE_MAX_WORK_ITEM_SIZES); }
            size_t max_work_group_size() const { return device_info<size_t>(my_cl_device_id, CL_DEVICE_MAX_WORK_GROUP_SIZE); }
            ...
            std::string built_in_kernels() const { return device_info<std::string>(my_cl_device_id, CL_DEVICE_BUILT_IN_KERNELS); }
            std::string name() const { return device_info<std::string>(my_cl_device_id, CL_DEVICE_NAME); }
            cl_bool available() const { return device_info<cl_bool>(my_cl_device_id, CL_DEVICE_AVAILABLE); }
            cl_bool compiler_available() const { return device_info<cl_bool>(my_cl_device_id, CL_DEVICE_COMPILER_AVAILABLE); }
            cl_bool linker_available() const { return device_info<cl_bool>(my_cl_device_id, CL_DEVICE_LINKER_AVAILABLE); }
            ...
            std::string extensions() const { return device_info<std::string>(my_cl_device_id, CL_DEVICE_EXTENSIONS); }

            cl_device_type type() const { return device_info<cl_device_type>(my_cl_device_id, CL_DEVICE_TYPE); }

            std::string vendor() const { return device_info<std::string>(my_cl_device_id, CL_DEVICE_VENDOR); }

            cl_uint address_bits() const { return device_info<cl_uint>(my_cl_device_id, CL_DEVICE_ADDRESS_BITS); }

            cl_device_id device_id() const { return my_cl_device_id; }

            cl_command_queue command_queue() const { return my_cl_command_queue; }

            void set_command_queue(cl_command_queue cmd_queue) { my_cl_command_queue = cmd_queue; }

        private:
            cl_platform_id platform() const { return device_info<cl_platform_id>(my_cl_device_id, CL_DEVICE_PLATFORM); }
            ...
            template <typename DeviceFilter> friend class opencl_factory;
            template <typename Factory> friend class opencl_memory;
            template <typename Factory> friend class opencl_program;

#if TBB_USE_ASSERT
            template <typename T, typename Factory> friend class opencl_buffer;
#endif
        };
        ...
        namespace internal {
            ...
            inline opencl_device_list find_available_devices() {
                ...
                for(auto d = devices.begin(); d != devices.end(); ++d) { opencl_devices.add(opencl_device((*d))); }
        ...
        template <typename Callback, typename T> class callback : public callback_base {
        ...
        template <typename T, typename Factory = opencl_info::default_opencl_factory> class opencl_async_msg : public async_msg<T> {
        public:
            ...
            opencl_async_msg(const opencl_async_msg& dmsg)
                ...
                if(my_is_event) enforce_cl_retcode(clRetainEvent(my_event), "Failed to retain an event");
            }
            ...
            opencl_async_msg& operator=(const opencl_async_msg& dmsg) {
                ...
                if(my_is_event) enforce_cl_retcode(clReleaseEvent(my_event), "Failed to retain an event");
            ...
            ~opencl_async_msg() {
                if(my_is_event) enforce_cl_retcode(clReleaseEvent(my_event), "Failed to release an event");
            }
            ...
            void set_event(cl_event e) const {
                if(my_is_event) {
                    ...
                    if(cq != event_info<cl_command_queue>(e, CL_EVENT_COMMAND_QUEUE)) enforce_cl_retcode(clFlush(cq), "Failed to flush an OpenCL command queue");
                    enforce_cl_retcode(clReleaseEvent(my_event), "Failed to release an event");
                }
                my_is_event = true;
                my_event = e;
            ...
            template <typename Callback> void register_callback(Callback c) const {
        ...
        protected:
            ...
            void finalize() const __TBB_override {
                ...
                if(!my_callback_flag_ptr->fetch_and_store(true)) {
                    ...
                    if(my_is_event) {
                        register_callback([a](const T& t) mutable { a.set(t); });
                    } else {
                        a.set(my_data);
                    }
                }
        ...
        template <typename K, typename T, typename Factory> K key_from_message(const opencl_async_msg<T, Factory>& dmsg) {
        ...
        template <typename Factory> class opencl_memory {
        public:
            opencl_memory() {}
        ...
        template <typename Factory> class opencl_buffer_impl : public opencl_memory<Factory> {
            size_t my_size;
        ...
        public:
            ...
            void map_memory(opencl_device device, opencl_async_msg<void*, Factory>& dmsg) __TBB_override { this->my_factory->enqueue_map_buffer(device, *this, dmsg); }

#if TBB_USE_ASSERT
            template <typename, typename> friend class opencl_buffer;
#endif
        };

        enum access_type { read_write, write_only, read_only };

        template <typename T, typename Factory = opencl_info::default_opencl_factory> class opencl_subbuffer;

        template <typename T, typename Factory = opencl_info::default_opencl_factory> class opencl_buffer {
        public:
            ...
            template <access_type a> iterator<a> access() const {
            ...
            template <access_type a = read_write> iterator<a> begin() const { return access<a>(); }

            template <access_type a = read_write> iterator<a> end() const { return access<a>() + my_impl->size() / sizeof(T); }
        ...
        private:
            ...
            opencl_buffer(Factory& f, cl_mem m, size_t index, size_t size)
                : my_impl(std::make_shared<impl_type>(m, index * sizeof(T), size * sizeof(T), f)) {}

            typedef opencl_buffer_impl<Factory> impl_type;

            std::shared_ptr<impl_type> my_impl;

            friend bool operator==(const opencl_buffer<T, Factory>& lhs, const opencl_buffer<T, Factory>& rhs) { return lhs.my_impl == rhs.my_impl; }

            template <typename> friend class opencl_factory;
            template <typename, typename> friend class opencl_subbuffer;
        };

        template <typename T, typename Factory> class opencl_subbuffer : public opencl_buffer<T, Factory> {
            opencl_buffer<T, Factory> my_owner;

        public:
            opencl_subbuffer() {}
            opencl_subbuffer(const opencl_buffer<T, Factory>& owner, size_t index, size_t size)
                : opencl_buffer<T, Factory>(*owner.my_impl->factory(), owner.native_object(), index, size)
                , my_owner(owner) {}
        };

        template <typename T, typename Factory> opencl_subbuffer<T, Factory> opencl_buffer<T, Factory>::subbuffer(size_t index, size_t size) const { return opencl_subbuffer<T, Factory>(*this, index, size); }

#define is_typedef(type)                                                      \
    template <typename T> struct is_##type {                                  \
        template <typename C> static std::true_type check(typename C::type*); \
        ...
        template <typename T> typename std::enable_if<is_native_object_type<T>::value, typename T::native_object_type>::type get_native_object(const T& t) { return t.native_object(); }

        template <typename T> typename std::enable_if<!is_native_object_type<T>::value, T>::type get_native_object(T t) { return t; }

        // send_if_memory_object checks if the T type has memory_object_type and call the send method for the object.
        template <typename T, typename Factory> typename std::enable_if<is_memory_object_type<T>::value>::type send_if_memory_object(opencl_device device, opencl_async_msg<T, Factory>& dmsg) {
            const T& t = dmsg.data(false);
        ...
        template <typename T> typename std::enable_if<is_memory_object_type<T>::value>::type send_if_memory_object(opencl_device device, T& t) {
            typedef typename T::memory_object_type mem_obj_t;
            mem_obj_t mem_obj = t.memory_object();
        ...
        template <typename T> typename std::enable_if<!is_memory_object_type<T>::value>::type send_if_memory_object(opencl_device, T&){};

        // receive_if_memory_object checks if the T type has memory_object_type and call the receive method for the object.
        template <typename T, typename Factory> typename std::enable_if<is_memory_object_type<T>::value>::type receive_if_memory_object(const opencl_async_msg<T, Factory>& dmsg) {
            const T& t = dmsg.data(false);
            typedef typename T::memory_object_type mem_obj_t;
            mem_obj_t mem_obj = t.memory_object();
            opencl_async_msg<mem_obj_t, Factory> d(mem_obj);
        ...
        template <typename T> typename std::enable_if<!is_memory_object_type<T>::value>::type receive_if_memory_object(const T&) {}
        ...
        public:
            ...
            template <typename G = std::initializer_list<int>, typename L = std::initializer_list<int>, typename = typename std::enable_if<!std::is_same<typename std::decay<G>::type, opencl_range>::value>::type> opencl_range(G&& global_work = std::initializer_list<int>({ 0 }), L&& local_work = std::initializer_list<int>({ 0, 0, 0 })) {
        ...
        public:
            ...
            public:
                ...
                ~kernel() { enforce_cl_retcode(clReleaseKernel(my_cl_kernel), "Failed to release a kernel"); }
            ...
            private:
                ...
                template <typename DeviceFilter_> friend class opencl_factory;
            ...
            ~opencl_factory() {
                if(my_devices.size()) {
                    for(auto d = my_devices.begin(); d != my_devices.end(); ++d) { enforce_cl_retcode(clReleaseCommandQueue((*d).my_cl_command_queue), "Failed to release a command queue"); }
        ...
        private:
            template <typename Factory> void enqueue_map_buffer(opencl_device device, opencl_buffer_impl<Factory>& buffer, opencl_async_msg<void*, Factory>& dmsg) {
                cl_event const* e1 = dmsg.get_event();
                cl_event e2;
                cl_int err;
                void* ptr = clEnqueueMapBuffer(device.my_cl_command_queue, buffer.get_cl_mem(), false, CL_MAP_READ | CL_MAP_WRITE, 0, buffer.size(), e1 == NULL ? 0 : 1, e1, &e2, &err);
            ...
            template <typename Factory> void enqueue_unmap_buffer(opencl_device device, opencl_memory<Factory>& memory, opencl_async_msg<void*, Factory>& dmsg) {
                cl_event const* e1 = dmsg.get_event();
                cl_event e2;
                enforce_cl_retcode(clEnqueueUnmapMemObject(device.my_cl_command_queue, memory.get_cl_mem(), memory.get_host_ptr(), e1 == NULL ? 0 : 1, e1, &e2), "Failed to unmap a buffer");
                dmsg.set_event(e2);
                enforce_cl_retcode(clReleaseEvent(e2), "Failed to release an event");
            }

            // --------- Kernel argument & event list helpers --------- //
            template <size_t NUM_ARGS, typename T> void process_one_arg(const kernel_type& kernel, std::array<cl_event, NUM_ARGS>&, int&, int& place, const T& t) {
            ...
            template <size_t NUM_ARGS, typename T, typename F> void process_one_arg(const kernel_type& kernel, std::array<cl_event, NUM_ARGS>& events, int& num_events, int& place, const opencl_async_msg<T, F>& msg) {
                ...
                if(e != NULL) { events[num_events++] = *e; }

                process_one_arg(kernel, events, num_events, place, msg.data(false));
            }

            template <size_t NUM_ARGS, typename T, typename... Rest> void process_arg_list(const kernel_type& kernel, std::array<cl_event, NUM_ARGS>& events, int& num_events, int& place, const T& t, const Rest&... args) {
                process_one_arg(kernel, events, num_events, place, t);
                process_arg_list(kernel, events, num_events, place, args...);
            }

            template <size_t NUM_ARGS> void process_arg_list(const kernel_type&, std::array<cl_event, NUM_ARGS>&, int&, int&) {}
            // ------------------------------------------- //
            template <typename T> void update_one_arg(cl_event, T&) {}

            template <typename T, typename F> void update_one_arg(cl_event e, opencl_async_msg<T, F>& msg) { msg.set_event(e); }

            template <typename T, typename... Rest> void update_arg_list(cl_event e, T& t, Rest&... args) {
                update_one_arg(e, t);
            ...
            // ------------------------------------------- //
        public:
            template <typename... Args> void send_kernel(opencl_device device, const kernel_type& kernel, const range_type& work_size, Args&... args) {
            ...
            // ------------------------------------------- //
            template <typename T, typename... Rest> void send_data(opencl_device device, T& t, Rest&... args) {
        ...
        private:
            cl_event send_kernel_impl(opencl_device device, const cl_kernel& kernel, const range_type& work_size, cl_uint num_events, cl_event* event_list) {
                ...
                cl_event event;
                enforce_cl_retcode(clEnqueueNDRangeKernel(device.my_cl_command_queue, kernel, s, g_offset.data(), g_size.data(), l_size[0] ? l_size.data() : NULL, num_events, num_events ? event_list : NULL, &event), "Failed to enqueue a kernel");
                return event;
            }

            // ------------------------------------------- //
            template <typename T> bool get_event_from_one_arg(cl_event&, const T&) { return false; }

            template <typename T, typename F> bool get_event_from_one_arg(cl_event& e, const opencl_async_msg<T, F>& msg) {
            ...
            template <typename T, typename... Rest> bool get_event_from_args(cl_event& e, const T& t, const Rest&... args) {
                if(get_event_from_one_arg(e, t)) { return true; }
            ...
            template <typename Fn> struct finalize_fn_leaf : public finalize_fn {
                Fn my_fn;
        ...
        public:
            template <typename FinalizeFn, typename... Args> void finalize(opencl_device device, FinalizeFn fn, Args&... args) {
                cl_event e;

                if(get_event_from_args(e, args...)) { enforce_cl_retcode(clSetEventCallback(e, CL_COMPLETE, finalize_callback, new finalize_fn_leaf<FinalizeFn>(fn)), "Failed to set a callback"); }
        ...
        private:
            bool is_same_context(opencl_device::device_id_type d1, opencl_device::device_id_type d2) {
                ...
                if(d1 != opencl_device::host && d2 != opencl_device::host) return true;
                return d1 == d2;
            }
        ...
        private:
            ...
            void init_once() {
                {
                    tbb::spin_mutex::scoped_lock lock(my_devices_mutex);
                    if(!my_devices.size()) my_devices = DeviceFilter()(opencl_info::available_devices());
                }
                ...
                for(opencl_device_list::iterator it = ++my_devices.begin(); it != my_devices.end(); ++it) enforce_cl_retcode(it->platform() == platform_id ? CL_SUCCESS : CL_INVALID_PLATFORM, "All devices should be in the same platform");

                std::vector<cl_device_id> cl_device_ids;
                for(auto d = my_devices.begin(); d != my_devices.end(); ++d) { cl_device_ids.push_back((*d).my_cl_device_id); }
                ...
                cl_int err;
                cl_context ctx = clCreateContext(context_properties, (cl_uint)cl_device_ids.size(), cl_device_ids.data(), NULL, NULL, &err);
...
#pragma GCC diagnostic pop
            ...
            template <typename Factory> friend class opencl_program;
            template <typename Factory> friend class opencl_buffer_impl;
            template <typename Factory> friend class opencl_memory;
        ...
        namespace opencl_info {
            ...
            template <typename Factory> struct default_device_selector {
        ...
        opencl_buffer<T, Factory>::opencl_buffer(size_t size)
            : my_impl(std::make_shared<impl_type>(size * sizeof(T), opencl_info::default_factory())) {}

        enum class opencl_program_type { SOURCE, PRECOMPILED, SPIR };

        template <typename Factory = opencl_info::default_opencl_factory> class opencl_program : tbb::internal::no_assign {
        public:
            ...
            kernel_type get_kernel(const std::string& k) const { return kernel_type(get_cl_kernel(k), my_factory); }

        private:
            opencl_program(Factory& factory, cl_program program)
                : my_factory(factory)
                , my_cl_program(program) {
                // Set my_do_once_flag to the called state.
                std::call_once(my_do_once_flag, []() {});
            }
            ...
            public:
                ...
                opencl_program_builder(Factory& f, const std::string& name, cl_program program, cl_uint num_devices, cl_device_id* device_list, const char* options, cl_callback_type callback, void* user_data) {
                    cl_int err = clBuildProgram(program, num_devices, device_list, options, callback, user_data);
                    ...
                    if(err == CL_BUILD_PROGRAM_FAILURE) {
                        ...
                        for(auto d = devices.begin(); d != devices.end(); ++d) {
                            ...
                            size_t log_size;
                            cl_int query_err = clGetProgramBuildInfo(program, (*d).my_cl_device_id, CL_PROGRAM_BUILD_LOG, 0, NULL, &log_size);
                            enforce_cl_retcode(query_err, "Failed to get build log size");
                            if(log_size) {
                                std::vector<char> output;
                                output.resize(log_size);
                                query_err = clGetProgramBuildInfo(program, (*d).my_cl_device_id, CL_PROGRAM_BUILD_LOG, output.size(), output.data(), NULL);
            ...
            public:
                template <typename Filter> opencl_device_filter(cl_uint& num_devices, cl_device_id* device_list, Filter filter, const char* message) {
                    for(cl_uint i = 0; i < num_devices; ++i)
                        if(filter(device_list[i])) { device_list[i--] = device_list[--num_devices]; }
                    if(!num_devices) enforce_cl_retcode(CL_DEVICE_NOT_AVAILABLE, message);
                }
            };

            void init(const std::string&) const {
                cl_uint num_devices;
                enforce_cl_retcode(clGetContextInfo(my_factory.context(), CL_CONTEXT_NUM_DEVICES, sizeof(num_devices), &num_devices, NULL), "Failed to get OpenCL context info");
                if(!num_devices) enforce_cl_retcode(CL_DEVICE_NOT_FOUND, "No supported devices found");
                cl_device_id* device_list = (cl_device_id*)alloca(num_devices * sizeof(cl_device_id));
                enforce_cl_retcode(clGetContextInfo(my_factory.context(), CL_CONTEXT_DEVICES, num_devices * sizeof(cl_device_id), device_list, NULL), "Failed to get OpenCL context info");
                ...
                case opencl_program_type::SOURCE: {
                    ...
                    opencl_device_filter(num_devices, device_list, [](const opencl_device& d) -> bool { return !d.compiler_available() || !d.linker_available(); }, "No one device supports building program from sources");
                    opencl_program_builder(my_factory, my_arg_str, my_cl_program, num_devices, device_list, options, /*callback*/ NULL, /*user data*/ NULL);
                    break;
                }
                ...
                case opencl_program_type::PRECOMPILED: {
                    file_reader fr(my_arg_str);
                    std::vector<const unsigned char*> s(num_devices, reinterpret_cast<const unsigned char*>(fr.content()));
                    std::vector<size_t> l(num_devices, fr.length());
                    std::vector<cl_int> bin_statuses(num_devices, -1);
                    cl_int err;
                    my_cl_program = clCreateProgramWithBinary(my_factory.context(), num_devices, device_list, l.data(), s.data(), bin_statuses.data(), &err);
                    if(err != CL_SUCCESS) {
                        std::string statuses_str;
                        for(auto st = bin_statuses.begin(); st != bin_statuses.end(); ++st) { statuses_str += std::to_string((*st)); }

                        enforce_cl_retcode(err, std::string("Failed to create program, error " + std::to_string(err) + " : ") + my_arg_str + std::string(", binary_statuses = ") + statuses_str);
                    }
                    opencl_program_builder(my_factory, my_arg_str, my_cl_program, num_devices, device_list, options, /*callback*/ NULL, /*user data*/ NULL);
                    break;
                }
            ...
            template <typename DeviceFilter> friend class opencl_factory<DeviceFilter>::kernel;
        };

        template <typename... Args> class opencl_node;

        template <typename JP, typename Factory, typename... Ports> class opencl_node<tuple<Ports...>, JP, Factory> : public streaming_node<tuple<Ports...>, JP, Factory> {
        ...
        template <typename JP, typename... Ports> class opencl_node<tuple<Ports...>, JP> : public opencl_node<tuple<Ports...>, JP, opencl_info::default_opencl_factory> {
        ...
        template <typename... Ports> class opencl_node<tuple<Ports...>> : public opencl_node<tuple<Ports...>, queueing, opencl_info::default_opencl_factory> {
