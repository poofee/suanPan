...
public:
    ...
    void blocking_terminate() { internal_blocking_terminate(/*throwing=*/true); }
#endif
    //! terminate() that waits for worker threads termination. Returns false on error.
    bool blocking_terminate(const std::nothrow_t&) __TBB_NOEXCEPT(true) { return internal_blocking_terminate(/*throwing=*/false); }
