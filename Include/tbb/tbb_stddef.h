...
#define __TBB_override // formal comment only
    ...
    template <class T, size_t S, size_t R> struct padded_base : T { char pad[S - R]; };
...
#else
    template <typename T> inline void poison_pointer(T* __TBB_atomic&) { /*do nothing*/
    }
#endif /* !TBB_USE_ASSERT */
...
namespace internal {
    ...
    template <typename T> struct allocator_type { typedef T value_type; };

#if _MSC_VER
    //! Microsoft std::allocator has non-standard extension that strips const from a type.
    template <typename T> struct allocator_type<const T> { typedef T value_type; };
#endif
    ...
    template <bool v> struct bool_constant { static /*constexpr*/ const bool value = v; };
...
#else
    template <typename allocator> struct allocator_traits { typedef tbb::internal::false_type propagate_on_container_move_assignment; };
#endif
    ...
    template <typename T> T& forward(T& x) { return x; }
