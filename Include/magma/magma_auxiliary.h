...
#ifdef HAVE_clBLAS
magma_int_t magma_init_opencl(cl_platform_id platform, cl_context context, magma_int_t setup_clBlas);

magma_int_t magma_finalize_opencl(magma_int_t finalize_clBlas);
#endif
...
#define magma_queue_create(device, queue_ptr) magma_queue_create_internal(device, queue_ptr, __func__, __FILE__, __LINE__)

#define magma_queue_create_from_cuda(device, cuda_stream, cublas_handle, cusparse_handle, queue_ptr) magma_queue_create_from_cuda_internal(device, cuda_stream, cublas_handle, cusparse_handle, queue_ptr, __func__, __FILE__, __LINE__)

#define magma_queue_create_from_opencl(device, cl_queue, queue_ptr) magma_queue_create_from_opencl_internal(device, cl_queue, queue_ptr, __func__, __FILE__, __LINE__)
...
#ifdef HAVE_clBLAS
magma_int_t magma_queue_create_from_opencl_internal(magma_device_t device, cl_command_queue cl_queue, const char* func, const char* file, int line);
#endif
...
__host__ __device__ static inline magma_int_t magma_ceildiv(magma_int_t x, magma_int_t y) { return (x + y - 1) / y; }
...
/// @ingroup magma_ceildiv
__host__ __device__ static inline magma_int_t magma_roundup(magma_int_t x, magma_int_t y) { return magma_ceildiv(x, y) * y; }
