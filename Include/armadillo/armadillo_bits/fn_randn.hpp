...
template <typename eT> arma_warn_unused inline typename arma_real_or_cx_only<eT>::result randn() { return eT(arma_rng::randn<eT>()); }

//! Generate a vector with all elements set to random values with a gaussian distribution (zero mean, unit variance)
arma_warn_unused arma_inline const Gen<vec, gen_randn> randn(const uword n_elem) {
...
arma_warn_unused arma_inline const Gen<mat, gen_randn> randn(const uword n_rows, const uword n_cols) {
    arma_extra_debug_sigprint();

    return Gen<mat, gen_randn>(n_rows, n_cols);
}

arma_warn_unused arma_inline const Gen<mat, gen_randn> randn(const SizeMat& s) {
    arma_extra_debug_sigprint();
...
arma_warn_unused arma_inline const GenCube<cube::elem_type, gen_randn> randn(const uword n_rows, const uword n_cols, const uword n_slices) {
    arma_extra_debug_sigprint();

    return GenCube<cube::elem_type, gen_randn>(n_rows, n_cols, n_slices);
}

arma_warn_unused arma_inline const GenCube<cube::elem_type, gen_randn> randn(const SizeCube& s) {
    arma_extra_debug_sigprint();
