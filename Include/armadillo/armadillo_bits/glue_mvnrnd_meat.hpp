...
template <typename T1, typename T2> inline void glue_mvnrnd::apply(Mat<typename T1::elem_type>& out, const Glue<T1, T2, glue_mvnrnd>& expr) {
...
template <typename T1, typename T2> inline bool glue_mvnrnd::apply_direct(Mat<typename T1::elem_type>& out, const Base<typename T1::elem_type, T1>& M, const Base<typename T1::elem_type, T2>& C, const uword N) {
    arma_extra_debug_sigprint();
...
template <typename eT> inline bool glue_mvnrnd::apply_noalias(Mat<eT>& out, const Mat<eT>& M, const Mat<eT>& C, const uword N) {
    arma_extra_debug_sigprint();
    ...
    if(chol_status == false) {
        ...
        for(uword i = 0; i < eigval_n_elem; ++i) {
            if(eigval_mem[i] < eT(0)) { eigval_mem[i] = eT(0); }
        }
