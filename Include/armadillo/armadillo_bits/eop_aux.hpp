...
public:
...
#else
    template <typename eT> arma_inline static typename arma_integral_only<eT>::result log2(const eT x) { return eT(std::log(double(x)) / double(0.69314718055994530942)); }
    template <typename eT> arma_inline static typename arma_real_or_cx_only<eT>::result log2(const eT x) {
        typedef typename get_pod_type<eT>::result T;
        return std::log(x) / T(0.69314718055994530942);
    }
#endif
...
#else
    template <typename eT> arma_inline static typename arma_integral_only<eT>::result exp2(const eT x) { return eT(std::pow(double(2), double(x))); }
    template <typename eT> arma_inline static typename arma_real_or_cx_only<eT>::result exp2(const eT x) {
        typedef typename get_pod_type<eT>::result T;
        return std::pow(T(2), x);
    }
#endif
    ...
    template <typename eT> arma_inline static typename arma_cx_only<eT>::result sign(const eT& x) {
...
#else
    template <typename eT> arma_inline static eT erf(const eT x) {
        arma_ignore(x);
        arma_stop_logic_error("erf(): C++11 compiler required");
        return eT(0);
    }
#endif
...
#else
    template <typename eT> arma_inline static eT erfc(const eT x) {
        arma_ignore(x);
        arma_stop_logic_error("erfc(): C++11 compiler required");
        return eT(0);
    }
#endif
...
#else
    template <typename eT> arma_inline static eT lgamma(const eT x) {
        arma_ignore(x);
        arma_stop_logic_error("lgamma(): C++11 compiler required");
        return eT(0);
    }
#endif
    ...
    template <typename eT> arma_inline static typename arma_integral_only<eT>::result direct_eps(const eT) { return eT(0); }

    template <typename eT> inline static typename arma_real_only<eT>::result direct_eps(const eT x) {
    ...
    template <typename T> inline static typename arma_real_only<T>::result direct_eps(const std::complex<T>& x) {
        // arma_extra_debug_sigprint();
