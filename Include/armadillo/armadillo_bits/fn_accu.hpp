...
template <typename T1> arma_warn_unused inline uword accu(const mtOp<uword, T1, op_rel_noteq>& X) {
    ...
    if(Proxy<T1>::use_at == false) {
        ...
        if(P_n_rows == 1) {
            for(uword col = 0; col < P_n_cols; ++col) { n_nonzero += (P.at(0, col) != val) ? uword(1) : uword(0); }
        } else {
...
template <typename T1> arma_warn_unused inline typename T1::elem_type accu(const SpBase<typename T1::elem_type, T1>& expr) {
    arma_extra_debug_sigprint();
