...
template <typename eT> inline void MapMat<eT>::print(const std::string& extra_text) const {
    ...
    get_cout_stream() << "[matrix size: " << n_rows << 'x' << n_cols << "; n_nonzero: " << n_nonzero << "; density: " << density << "%]\n\n";
...
template <typename eT> inline void MapMat<eT>::init_warm(const uword in_n_rows, const uword in_n_cols) {
    ...
    arma_debug_check((((in_n_rows > ARMA_MAX_UHWORD) || (in_n_cols > ARMA_MAX_UHWORD)) ? ((double(in_n_rows) * double(in_n_cols)) > double(ARMA_MAX_UWORD)) : false), error_message);
...
template <typename eT>
arma_inline SpMat_MapMat_val<eT>::SpMat_MapMat_val(SpMat<eT>& in_s_parent, MapMat<eT>& in_m_parent, const uword in_row, const uword in_col)
...
template <typename eT> arma_inline SpMat_MapMat_val<eT>::operator eT() const {
    arma_extra_debug_sigprint();
...
template <typename eT> inline SpMat_MapMat_val<eT>& SpMat_MapMat_val<eT>::operator=(const SpMat_MapMat_val<eT>& x) {
    arma_extra_debug_sigprint();
...
template <typename eT> inline SpMat_MapMat_val<eT>& SpMat_MapMat_val<eT>::operator=(const eT in_val) {
    arma_extra_debug_sigprint();
...
template <typename eT> inline SpMat_MapMat_val<eT>& SpMat_MapMat_val<eT>::operator++() {
    arma_extra_debug_sigprint();
...
template <typename eT>
arma_inline SpSubview_MapMat_val<eT>::SpSubview_MapMat_val(SpSubview<eT>& in_v_parent, MapMat<eT>& in_m_parent, const uword in_row, const uword in_col)
...
template <typename eT> arma_inline void SpSubview_MapMat_val<eT>::update_n_nonzeros() {
    arma_extra_debug_sigprint();
    ...
    if(v_parent.m.n_nonzero > old_n_nonzero) {
        access::rw(v_parent.n_nonzero)++;
    } else if(v_parent.m.n_nonzero < old_n_nonzero) {
        access::rw(v_parent.n_nonzero)--;
    }
}

template <typename eT> arma_inline SpSubview_MapMat_val<eT>::operator eT() const {
    arma_extra_debug_sigprint();
...
template <typename eT> inline SpSubview_MapMat_val<eT>& SpSubview_MapMat_val<eT>::operator=(const SpSubview_MapMat_val<eT>& x) {
    arma_extra_debug_sigprint();
...
template <typename eT> inline SpSubview_MapMat_val<eT>& SpSubview_MapMat_val<eT>::operator=(const eT in_val) {
    arma_extra_debug_sigprint();
...
template <typename eT> inline SpSubview_MapMat_val<eT>& SpSubview_MapMat_val<eT>::operator++() {
    arma_extra_debug_sigprint();
