...
template <typename T1> inline void op_reverse_vec::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_reverse_vec>& in) {
    ...
    if(is_Mat<typename Proxy<T1>::stored_type>::value || P.is_alias(out)) {
        ...
        } else if((T1::is_row) || (P.get_n_rows() == 1)) {
...
template <typename T1> inline void op_reverse_mat::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_reverse_mat>& in) {
    arma_extra_debug_sigprint();
