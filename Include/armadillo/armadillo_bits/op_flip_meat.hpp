...
template <typename eT> inline void op_flipud::apply_direct(Mat<eT>& out, const Mat<eT>& X) {
    arma_extra_debug_sigprint();
...
template <typename T1> inline void op_flipud::apply_proxy_noalias(Mat<typename T1::elem_type>& out, const Proxy<T1>& P) {
    arma_extra_debug_sigprint();
...
template <typename eT> inline void op_fliplr::apply_direct(Mat<eT>& out, const Mat<eT>& X) {
    arma_extra_debug_sigprint();
    ...
    if(&out != &X) {
        ...
        if(X_n_rows == 1) {
            ...
            for(uword col = 0; col < X_n_cols; ++col) { out.col(X_n_cols_m1 - col) = X.col(col); }
        }
    } else // in-place operation
    {
        ...
        if(X_n_rows == 1) {
            ...
            for(uword col = 0; col < N; ++col) { out.swap_cols(X_n_cols_m1 - col, col); }
        }
    }
}

template <typename T1> inline void op_fliplr::apply_proxy_noalias(Mat<typename T1::elem_type>& out, const Proxy<T1>& P) {
    arma_extra_debug_sigprint();
