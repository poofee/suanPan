...
template <typename eT> inline void spop_strans::apply_noalias(SpMat<eT>& B, const SpMat<eT>& A) {
    ...
    for(uword i = 0; i < n; ++i) {
        for(uword j = ia[i]; j < ia[i + 1]; ++j) { ib[ja[j] + 1]++; }
    }
...
template <typename T1> inline void spop_strans::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_strans>& in) {
    arma_extra_debug_sigprint();
...
template <typename T1> inline void spop_strans::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_htrans>& in) {
    arma_extra_debug_sigprint();
