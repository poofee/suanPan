...
template <typename obj_type> arma_warn_unused inline obj_type randi(const uword n_rows, const uword n_cols, const distr_param& param = distr_param(), const typename arma_Mat_Col_Row_only<obj_type>::result* junk = 0) {
...
template <typename obj_type> arma_warn_unused inline obj_type randi(const SizeMat& s, const distr_param& param = distr_param(), const typename arma_Mat_Col_Row_only<obj_type>::result* junk = 0) {
    arma_extra_debug_sigprint();
    arma_ignore(junk);
...
template <typename obj_type> arma_warn_unused inline obj_type randi(const uword n_elem, const distr_param& param = distr_param(), const arma_empty_class junk1 = arma_empty_class(), const typename arma_Mat_Col_Row_only<obj_type>::result* junk2 = 0) {
    arma_extra_debug_sigprint();
    arma_ignore(junk1);
    arma_ignore(junk2);
...
template <typename eT> arma_warn_unused inline typename arma_scalar_only<eT>::result randi(const distr_param& param) { return eT(as_scalar(randi<Col<eT>>(uword(1), uword(1), param))); }

arma_warn_unused inline sword randi() { return sword(arma_rng::randi<sword>()); }

template <typename eT> arma_warn_unused inline typename arma_scalar_only<eT>::result randi() { return eT(arma_rng::randi<eT>()); }

template <typename cube_type> arma_warn_unused inline cube_type randi(const uword n_rows, const uword n_cols, const uword n_slices, const distr_param& param = distr_param(), const typename arma_Cube_only<cube_type>::result* junk = 0) {
    arma_extra_debug_sigprint();
    arma_ignore(junk);
...
template <typename cube_type> arma_warn_unused inline cube_type randi(const SizeCube& s, const distr_param& param = distr_param(), const typename arma_Cube_only<cube_type>::result* junk = 0) {
    arma_extra_debug_sigprint();
    arma_ignore(junk);
