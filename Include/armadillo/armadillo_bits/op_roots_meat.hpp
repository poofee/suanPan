...
template <typename T1> inline void op_roots::apply(Mat<std::complex<typename T1::pod_type>>& out, const mtOp<std::complex<typename T1::pod_type>, T1, op_roots>& expr) {
...
template <typename T1> inline bool op_roots::apply_direct(Mat<std::complex<typename T1::pod_type>>& out, const Base<typename T1::elem_type, T1>& X) {
    arma_extra_debug_sigprint();
...
template <typename eT> inline bool op_roots::apply_noalias(Mat<std::complex<typename get_pod_type<eT>::result>>& out, const Mat<eT>& X) {
    arma_extra_debug_sigprint();
