...
#define arma_applier_1u(operatorA)                                                                       \
    {                                                                                                    \
        for(uword i = 0; i < n_elem; ++i) { out_mem[i] operatorA eop_core<eop_type>::process(P[i], k); } \
    }
#else
#define arma_applier_1u(operatorA)                                                    \
    {                                                                                 \
        ...
        for(i = 0, j = 1; j < n_elem; i += 2, j += 2) {                               \
            ...
            out_mem[j] operatorA tmp_j;                                               \
                                                                                      ...
                                                                                      \
        if(i < n_elem) { out_mem[i] operatorA eop_core<eop_type>::process(P[i], k); } \
    }
#endif
...
#define arma_applier_1a(operatorA)                                                                              \
    {                                                                                                           \
        for(uword i = 0; i < n_elem; ++i) { out_mem[i] operatorA eop_core<eop_type>::process(P.at_alt(i), k); } \
    }
#else
#define arma_applier_1a(operatorA)                                                           \
    {                                                                                        \
        ...
        for(i = 0, j = 1; j < n_elem; i += 2, j += 2) {                                      \
            ...
            out_mem[j] operatorA tmp_j;                                                      \
                                                                                             ...
                                                                                             \
        if(i < n_elem) { out_mem[i] operatorA eop_core<eop_type>::process(P.at_alt(i), k); } \
    }
#endif
...
#define arma_applier_2(operatorA)                                                                                                      \
    {                                                                                                                                  \
        if(n_rows != 1) {                                                                                                              \
            ...
            for(uword count = 0; count < n_cols; ++count) { out_mem[count] operatorA eop_core<eop_type>::process(P.at(0, count), k); } \
        }                                                                                                                              \
    }
...
#define arma_applier_1_mp(operatorA)                                                                                                                                         \
    {                                                                                                                                                                        \
        ...
        _Pragma("omp parallel for schedule(static) num_threads(n_threads)") for(uword i = 0; i < n_elem; ++i) { out_mem[i] operatorA eop_core<eop_type>::process(P[i], k); } \
    }
...
#define arma_applier_2_mp(operatorA)                                                                                                                                                                       \
    {                                                                                                                                                                                                      \
        const int n_threads = mp_thread_limit::get();                                                                                                                                                      \
        if(n_cols == 1) {                                                                                                                                                                                  \
            _Pragma("omp parallel for schedule(static) num_threads(n_threads)") for(uword count = 0; count < n_rows; ++count) { out_mem[count] operatorA eop_core<eop_type>::process(P.at(count, 0), k); } \
        } else if(n_rows == 1) {                                                                                                                                                                           \
            _Pragma("omp parallel for schedule(static) num_threads(n_threads)") for(uword count = 0; count < n_cols; ++count) { out_mem[count] operatorA eop_core<eop_type>::process(P.at(0, count), k); } \
        } else {                                                                                                                                                                                           \
            _Pragma("omp parallel for schedule(static) num_threads(n_threads)") for(uword col = 0; col < n_cols; ++col) {                                                                                  \
                for(uword row = 0; row < n_rows; ++row) { out.at(row, col) operatorA eop_core<eop_type>::process(P.at(row, col), k); }                                                                     \
            }                                                                                                                                                                                              \
        }                                                                                                                                                                                                  \
    }
...
#define arma_applier_3_mp(operatorA)                                                                                                                 \
    {                                                                                                                                                \
        ...
        _Pragma("omp parallel for schedule(static) num_threads(n_threads)") for(uword slice = 0; slice < n_slices; ++slice) {                        \
            for(uword col = 0; col < n_cols; ++col)                                                                                                  \
                for(uword row = 0; row < n_rows; ++row) { out.at(row, col, slice) operatorA eop_core<eop_type>::process(P.at(row, col, slice), k); } \
        }                                                                                                                                            \
    }
