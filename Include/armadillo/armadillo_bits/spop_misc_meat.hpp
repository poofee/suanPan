...
template <typename T1> inline void spop_repelem::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_repelem>& in) {
    arma_extra_debug_sigprint();
...
template <typename T1> inline void spop_diagvec::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_diagvec>& in) {
    arma_extra_debug_sigprint();
    ...
    arma_debug_check(((row_offset > 0) && (row_offset >= X.n_rows)) || ((col_offset > 0) && (col_offset >= X.n_cols)), "diagvec(): requested diagonal out of bounds");
...
template <typename T1> inline void spop_flipud::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_flipud>& in) {
    arma_extra_debug_sigprint();

    out = reverse(in.m, 0);
}

template <typename T1> inline void spop_fliplr::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_fliplr>& in) {
    arma_extra_debug_sigprint();
