...
template <typename T1> inline void spop_repmat::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_repmat>& X) {
...
template <typename eT> inline void spop_repmat::apply_noalias(SpMat<eT>& out, const uword A_n_rows, const uword A_n_cols, const SpMat<eT>& B) {
    arma_extra_debug_sigprint();
