...
inline herr_t hdf5_search_callback(hid_t loc_id,
    const char* name,
...
inline hid_t search_hdf5_file(const std::vector<std::string>& names, hid_t hdf5_file, int num_dims = 2, bool exact = false) {
...
struct hdf5_suspend_printing_errors {
    ...
    inline ~hdf5_suspend_printing_errors() { arma_H5Eset_auto(H5E_DEFAULT, old_client_func, old_client_data); }
