...
template <typename T1> inline void spop_normalise::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_normalise>& expr) {
...
template <typename eT> inline void spop_normalise::apply_direct(SpMat<eT>& out, const SpMat<eT>& X, const uword p, const uword dim) {
    arma_extra_debug_sigprint();
