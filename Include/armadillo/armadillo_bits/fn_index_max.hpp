...
template <typename T1> arma_warn_unused arma_inline const mtOpCube<uword, T1, op_index_max> index_max(const BaseCube<typename T1::elem_type, T1>& X, const uword dim = 0) {
    arma_extra_debug_sigprint();
...
arma_warn_unused inline uword index_max(const SizeMat& s) { return (s.n_rows >= s.n_cols) ? uword(0) : uword(1); }
