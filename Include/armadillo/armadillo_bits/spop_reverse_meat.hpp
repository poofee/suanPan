...
template <typename eT> inline void spop_reverse::apply_spmat(SpMat<eT>& out, const SpMat<eT>& X, const uword dim) {
...
template <typename T1> inline void spop_reverse::apply_proxy(SpMat<typename T1::elem_type>& out, const T1& X, const uword dim) {
    arma_extra_debug_sigprint();
...
template <typename T1> inline void spop_reverse::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_reverse>& in) {
    arma_extra_debug_sigprint();
