...
template <typename T1> inline void op_chol::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_chol>& X) {
...
template <typename T1> inline bool op_chol::apply_direct(Mat<typename T1::elem_type>& out, const Base<typename T1::elem_type, T1>& A_expr, const uword layout) {
    arma_extra_debug_sigprint();
