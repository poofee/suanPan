    ...
    template <typename eT, typename TA, typename TB> inline static void apply_blas_type(Mat<eT>& C, const TA& A, const TB& B, const eT alpha = eT(1), const eT beta = eT(0)) {
        ...
        if((A.n_rows <= 4) && (A.n_rows == A.n_cols) && (A.n_rows == B.n_rows) && (B.n_rows == B.n_cols) && (is_cx<eT>::no)) {
...
#if defined(ARMA_USE_ATLAS)
            {
                ...
                atlas::cblas_gemm<eT>(atlas::CblasColMajor, (do_trans_A) ? (is_cx<eT>::yes ? CblasConjTrans : atlas::CblasTrans) : atlas::CblasNoTrans, (do_trans_B) ? (is_cx<eT>::yes ? CblasConjTrans : atlas::CblasTrans) : atlas::CblasNoTrans, C.n_rows, C.n_cols, (do_trans_A) ? A.n_rows : A.n_cols, (use_alpha) ? alpha : eT(1), A.mem, (do_trans_A) ? A.n_rows : C.n_rows, B.mem, (do_trans_B) ? C.n_cols : ((do_trans_A) ? A.n_rows : A.n_cols), (use_beta) ? beta : eT(0), C.memptr(), C.n_rows);
            }
#elif defined(ARMA_USE_BLAS)
            {
                ...
                blas::gemm<eT>(&trans_A, &trans_B, &m, &n, &k, &local_alpha, A.mem, &lda, B.mem, &ldb, &local_beta, C.memptr(), &m);
            }
#else
