...
const unsigned RGCMS::m_node = 4;
const unsigned RGCMS::m_dof = 3;
const unsigned RGCMS::m_size = m_dof * m_node;
mat RGCMS::iso_mapping;
 ...
 */
RGCMS::ResultantConverter::ResultantConverter(const unsigned E, const double T, const vector<weak_ptr<Node>>& NP, const IntegrationPlan& IP, const mat& TRANS)
...
double RGCMS::ResultantConverter::F(const vec& alpha) const { return dot(direction_cosine.row(0), converter_a * alpha); }

double RGCMS::ResultantConverter::V(const vec& alpha) const { return dot(direction_cosine.row(1), converter_a * alpha); }

double RGCMS::ResultantConverter::M(const vec& alpha) const { return dot(direction_cosine.row(0), converter_b * alpha); }

RGCMS::IntegrationPoint::IntegrationPoint(vec C, const double F, unique_ptr<Material>&& M)
    : coor(std::move(C))
    , factor(F)
    , m_material(std::move(M)) {}

mat RGCMS::form_interpolation_displacement(const mat& pn_pxy, const mat& pnt_pxy) {
...
mat RGCMS::form_interpolation_enhanced_strain(const mat& pn_pxy) {
...
RGCMS::RGCMS(const unsigned T, const uvec& N, const unsigned M, const double TH, const char IP, const double RHOX, const unsigned MATX, const double RHOY, const unsigned MATY)
    : MaterialElement(T, ET_RGCMS, m_node, m_dof, N, uvec{ M })
    , thickness(TH)
    , int_scheme(IP)
    , rho_x(RHOX)
    , rho_y(RHOY)
...
void RGCMS::initialize(const shared_ptr<DomainBase>& D) {
...
vector<vec> RGCMS::record(const OutputType& T) {
    vector<vec> data;
...
void RGCMS::print() {
    suanpan_info("RGCMS mixed quad element %u connects nodes:\n", get_tag());
