...
mat RGCMQ::form_interpolation_displacement(const mat& pn_pxy, const mat& pnt_pxy) {
...
mat RGCMQ::form_interpolation_enhanced_strain(const mat& pn_pxy) {
...
RGCMQ::RGCMQ(const unsigned T, const uvec& N, const unsigned M, const double TH, const char IP, const double RHOX, const unsigned MATX, const double RHOY, const unsigned MATY)
    : MaterialElement(T, ET_RGCMQ, m_node, m_dof, N, uvec{ M })
    , thickness(TH)
    , int_scheme(IP)
    , rho_x(RHOX)
    , rho_y(RHOY)
...
void RGCMQ::initialize(const shared_ptr<DomainBase>& D) {
    ...
    for(unsigned I = 0; I < plan.n_rows; ++I) {
        ...
        int_pt[I].poly_strain = int_pt[I].poly_stress;
