...
const unsigned GCMT::m_node = 4;
const unsigned GCMT::m_dof = 3;
const unsigned GCMT::m_size = m_dof * m_node;
mat GCMT::mapping;
 ...
 */
GCMT::ResultantConverter::ResultantConverter(const unsigned E, const double T, const vector<weak_ptr<Node>>& NP, const IntegrationPlan& IP, const mat& TRANS)
...
double GCMT::ResultantConverter::F(const vec& alpha) const { return dot(direction_cosine.row(0), converter_a * alpha); }

double GCMT::ResultantConverter::V(const vec& alpha) const { return dot(direction_cosine.row(1), converter_a * alpha); }

double GCMT::ResultantConverter::M(const vec& alpha) const { return dot(direction_cosine.row(0), converter_b * alpha); }

GCMT::IntegrationPoint::IntegrationPoint(vec C, const double F, unique_ptr<Material>&& M)
    : coor(std::move(C))
    , factor(F)
    , m_material(std::move(M)) {}

mat GCMT::form_interpolation_displacement(const mat& pn_pxy, const mat& pnt_pxy) {
...
mat GCMT::form_interpolation_enhanced_strain(const mat& pn_pxy) {
...
GCMT::GCMT(const unsigned T, const uvec& N, const unsigned M, const double TH, const char IP)
    : MaterialElement(T, ET_GCMT, m_node, m_dof, N, uvec{ M })
    , thickness(TH)
    , int_scheme(IP) {
    if(mapping.is_empty()) {
        mat t_mapping(4, 4);
...
void GCMT::initialize(const shared_ptr<DomainBase>& D) {
    ...
    for(unsigned I = 0; I < plan.n_rows; ++I) {
        ...
        const auto t_mat = int_pt[I].poly_stress.t() * int_pt[I].factor;
...
vector<vec> GCMT::record(const OutputType& T) {
    vector<vec> data;
    switch(T) {
    case OutputType::S:
...
void GCMT::print() {
    suanpan_info("GCMT mixed quad element %u connects nodes:\n", get_tag());
