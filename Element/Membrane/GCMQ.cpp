...
mat GCMQ::form_interpolation_displacement(const mat& pn_pxy, const mat& pnt_pxy) {
...
mat GCMQ::form_interpolation_enhanced_strain(const mat& pn_pxy) {
...
GCMQ::GCMQ(const unsigned T, const uvec& N, const unsigned M, const double TH, const char IP)
    : MaterialElement(T, ET_GCMQ, m_node, m_dof, N, uvec{ M })
    , thickness(TH)
    , int_scheme(IP) {
    if(mapping.is_empty()) {
        mat t_mapping(4, 4);
