...
CP4::IntegrationPoint::IntegrationPoint(vec C, const double W, const double J, unique_ptr<Material>&& M, mat PNPXY)
    : coor(std::move(C))
    , weight(W)
    , jacob_det(J)
...
Op<vec, op_diagmat> CP4::get_diag_mass() const { return suanpan::mass::lumped_scale::apply(trial_mass, m_dof); }
