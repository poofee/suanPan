...
int eig_solve(vec& eigval, mat& eigvec, const std::shared_ptr<BandSymmMat<double>>& K, const std::shared_ptr<BandSymmMat<double>>& M, const unsigned num, const char* form) {
    auto IDO = 0;
    auto BMAT = 'G'; // generalized eigenvalue problem A*x=lambda*M*x
    auto N = static_cast<int>(K->n_rows);
    char WHICH[2];
...
int eig_solve(vec& eigval, mat& eigvec, const std::shared_ptr<SymmPackMat<double>>& K, const std::shared_ptr<SymmPackMat<double>>& M, const unsigned num, const char* form) {
    auto IDO = 0;
    auto BMAT = 'G'; // generalized eigenvalue problem A*x=lambda*M*x
    auto N = static_cast<int>(K->n_rows);
    char WHICH[2];
